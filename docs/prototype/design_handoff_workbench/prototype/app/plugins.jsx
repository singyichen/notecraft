...
const DATAFILES = [
  {
    id: "trendmile-schema",
    path: "planning/81-schema-er-diagram.er.json",
  ...
  {
    id: "notes-schema-mini",
    path: "notes-schema.er.json",
    route: "/view/notes-schema",
    title: "筆記系統資料表小抄",
    description: "NoteCraft 自身的 6 張表與關聯，放在筆記根目錄方便隨手改。",
    plugin: "er-diagram-renderer",
    pluginLabel: "ER 關聯圖",
    updatedAt: "2026-06-11",
    source: "notes-schema.er.json",
  },
];
...
function DataFileView({ fileId, devMode, viewWidth, vizError, onBack, onOpenNote, onOpenSeries }) {
  ...
  // plugin 渲染頁只呈現資料本身：不進入閱讀進度、不顯示系列導覽與「看完了嗎」
  ...
  return h("div", { style: { animation: "ncFade var(--duration-normal) var(--ease-out)" } },
    ...
    // ── 渲染區：佔滿內容區，不套外框卡片 ──
    h("div", { style: { minHeight: "calc(100vh - 118px)", background: "var(--surface-page)", padding: "18px clamp(12px, 3vw, 40px) 48px" } },
      ...
      shown.map((f) => h(Card, { key: f.id, hoverable: true, style: { padding: "16px 20px", cursor: "pointer" }, onClick: () => onOpen(f.id) },
        h("div", { style: { display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" } },
          ...
          h("div", { style: { minWidth: 0, flex: "1 1 220px" } },
            h("h3", { style: { fontSize: 16.5, color: V.ink, margin: "0 0 4px", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, f.title),
          ...
          h("div", { style: { display: "flex", alignItems: "center", gap: 16, flex: "none", marginLeft: "auto" } },
  ...
  if (layout === "list") {
    return h(Card, { hoverable: true, style: { padding: "16px 20px", cursor: "pointer" }, onClick: () => onOpen(file.id) },
      h("div", { style: { display: "flex", alignItems: "center", gap: 18 } },
        h(DataMark, null),
        h("div", { style: { minWidth: 0, flex: 1 } },
          h("h3", { style: { fontSize: 16.5, color: V.ink, margin: "0 0 4px", fontWeight: 700, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" } }, file.title),
