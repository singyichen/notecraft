# 產生 Lab 1 實驗四「第 26 頁：橋式加 1 µF 濾波電容」的 KiCad 原理圖。
# 用法（在本目錄）：python3 gen_exp4_rc.py lab1-exp4-bridge-rc.kicad_sch
# 和 gen_exp4.py 只差一顆與 R_L 並聯的 C1；Tinkercad 的求解器跑不動這一版，
# 筆記的電流分析圖就以這張原理圖當底圖。
import uuid, sys, os
...
XC=120.65            # 濾波電容那一欄（第 26 頁才並上去）
XR=139.7             # 負載那一欄（往右挪，讓 D4 與 C1 的標註不重疊）
YT=45.72             # 上節點 A
YM=60.96             # 兩個分支的中點
YB=76.2              # 下節點 B
YG=96.52             # 接地線
...
C1=(XC,72.39)        # 上 (XC,68.58)  下 (XC,76.2)
...
items+=wire((XB,YM),(XC,YM),(XR,YM),(XR,68.58))  # P → 濾波電容上端 → 負載上端
items+=wire((XC,YM),(XC,68.58))                  # P → C1 上端
items+=wire((XC,76.2),(XC,YG))                   # C1 下端 → 接地線
items+=wire((XR,76.2),(XR,YG),(XC,YG),(GND[0],YG))  # 負載下端 → 接地
items+=[junction(XA,YT),junction(XA,YB),junction(XA,YM),junction(XB,YM),junction(GND[0],YG),
        junction(XC,YM),junction(XC,YG)]
# labels
...
items.append(text('Lab 1 實驗四（第 26 頁）：全波橋式整流器 + 1 µF 濾波電容',20.32,25.4,size=2.5,justify='left bottom'))
items.append(text('正半週 A→D3→P→R_{L}→N→D1→B；負半週 B→D4→P→R_{L}→N→D2→A',20.32,110,size=FS,justify='left bottom'))
items.append(text('V_{out,peak} ≈ V_{in,peak} − 2V_{D,on}',20.32,114,size=FS,justify='left bottom'))
items.append(text('C1 與 R_{L} 並聯：Vin 高於電容電壓時充電，低於時由電容單獨供應負載',20.32,118,size=1.27,justify='left bottom'))
...
doc=['kicad_sch',['version','20250114'],['generator','"eeschema"'],['generator_version','"9.0"'],['uuid',ROOT],
     ['paper','"A5"'],
     ['title_block',['title',q('Lab 1 實驗四：全波橋式整流器 + RC 濾波')],['date','"2026-09-25"'],['rev','"1"'],['company',q('NoteCraft 電子學實作系列')]],
