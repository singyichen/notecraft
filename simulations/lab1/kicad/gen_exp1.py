# 產生 Lab 1 實驗一的 KiCad 原理圖（lab1-exp1-forward.kicad_sch）。
# 用法（在本目錄）：python3 gen_exp1.py lab1-exp1-forward.kicad_sch
import uuid, sys, os
...
D1=(XR,48.26)      # rot 90: A top (XR,44.45) K bottom (XR,52.07)
...
items.append(text('0–2 V',XL-3.81,V1[1]+3.175,size=1.27,justify='right bottom'))
items.append(text('+',XM+1.27,43.18-0.635,size=FS))
items.append(text('−',XM+1.27,53.34+2.54,size=FS))
items.append(text('Lab 1 實驗一：順偏導通電壓量測電路',20.32,25.4,size=2.5,justify='left bottom'))
items.append(text('V_{s} = V_{D} + I·1 kΩ',20.32,105,size=FS,justify='left bottom'))
...
doc=['kicad_sch',['version','20250114'],['generator','"eeschema"'],['generator_version','"9.0"'],['uuid',ROOT],
     ['paper','"A5"'],
     ['title_block',['title',q('Lab 1 實驗一：順偏導通電壓')],['date','"2026-09-19"'],['rev','"1"'],['company',q('NoteCraft 電子學實作系列')]],
