...
description: Use when explaining, writing, or reviewing any electronics / semiconductor-physics content in NoteCraft（電子學實作系列筆記、Razavi Fundamentals of Microelectronics 教材）for an author with a mathematics degree and no prior physics or circuits coursework — including 補地基、解釋物理定義與原理、為什麼公式長這樣、幫我看懂這段講義、電場／電位／kT／載子／PN 接面／二極體方程式看不懂, or writing @ai-visualize prompts for physics concepts. Also triggers on "explain this electronics concept from a math background" / "physics prerequisites for this note".
---
...
文字風格維持筆記既有的精確、表格／公式導向；只借物理直覺，不寫成講課口語。長段散文要拆成表格或條列。
