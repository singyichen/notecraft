...
|---|---|---|---|
| `paper` | Page background, default node fill | `#F5F3FF` (violet-50) | `#0B0B12` (near-black) |
| `paper-2` | Diagram container bg, secondary fill | `#FFFFFF` | `#16161F` |
| `ink` | Primary text, primary stroke | `#1E1B4B` (indigo-950) | `#E2E8F0` (slate-200) |
| `muted` | Secondary text, default arrow stroke | `#64748B` (slate-500) | `#A1A1AA` (zinc-400) |
| `soft` | Sublabels, boundary labels | `#94A3B8` (slate-400) | `#9CA3AF` |
| `rule` | Hairline borders | `rgba(30,27,75,0.12)` | `rgba(226,232,240,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#E2E8F0` (slate-200) | `#2A2A35` |
| `accent` | Focal / 1–2 max per diagram | `#6366F1` (indigo-500) | `#818CF8` (indigo-400) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(99,102,241,0.08)` | `rgba(129,140,248,0.10)` |
| `link` | HTTP/API calls, external arrows | `#818CF8` (indigo-400) | `#A5B4FC` (indigo-300) |

## Active skin

**ACTIVE SKIN = Label Suite.** Source of truth: `design/prototype/assets/tokens.css` in the `label-suite` repo (derived from `design/system/MASTER.md`). Restore upstream defaults by copying `style-guide.default-backup.md` over this file, or by loading the `default` profile.

| Role | tokens.css variable | Light | Dark |
|---|---|---|---|
| `paper` | `--color-surface` | `#F5F3FF` | `#0B0B12` |
| `paper-2` | `--color-white` | `#FFFFFF` | `#16161F` |
| `ink` | `--color-ink` | `#1E1B4B` | `#E2E8F0` |
| `muted` | `--color-text-soft` | `#64748B` | `#A1A1AA` |
| `soft` | `--color-ink-muted` | `#94A3B8` | `#9CA3AF` |
| `rule-solid` | `--color-border` | `#E2E8F0` | `#2A2A35` |
| `accent` | `--color-primary` | `#6366F1` | `#818CF8` |
| `link` | `--color-secondary` | `#818CF8` | `#A5B4FC` |

Dark values come from the `html[data-theme="dark"]` block of `tokens.css`, not from the upstream inversion rule. Two roles are **derived**, not named in `tokens.css`: `rule` is `ink` at 12% opacity and `accent-tint` is `accent` at 8% (light) / 10% (dark), both following the upstream pattern.

**Known deviation:** Label Suite's palette has no distinct link hue, so `link` maps to `--color-secondary`, which sits close to `accent`. Use it for arrow strokes only (its stated purpose); do not use it for body text, where the contrast against `paper` is insufficient.
...
|---|---|---|---|---|
| `title` | Crimson Pro | 1.75rem | 400 | Page H1 |
...
| `callout` | Crimson Pro *italic* | 14px | 400 | Editorial asides only |

### Font stack

```html
<link href="https://fonts.googleapis.com/css2?family=Crimson+Pro:ital,wght@0,400;0,600;1,400&family=Inter:wght@400;500;600;700&family=JetBrains+Mono:wght@400;500;600&family=Noto+Sans+TC:wght@400;500;700&family=Noto+Serif+TC:wght@400;600&display=swap" rel="stylesheet">
```

**Traditional Chinese fallback is required.** Diagrams for product users are written in Traditional Chinese; without `Noto Sans TC` / `Noto Serif TC` the glyphs fall back to system fonts and the figure stops matching the product UI. CSS font stacks:

```css
--font-serif: 'Crimson Pro', 'Noto Serif TC', Georgia, serif;
--font-sans:  'Inter', 'Noto Sans TC', 'PingFang TC', -apple-system, sans-serif;
--font-mono:  'JetBrains Mono', ui-monospace, SFMono-Regular, Menlo, monospace;
```

**Load-bearing rule:** Mono is for *technical* content (ports, commands, URLs, field types). Names go in Inter sans. Page title is Crimson Pro. Italic Crimson Pro is reserved for annotation callouts (see [primitive-annotation.md](primitive-annotation.md)).

> **Upstream rule waived in this skin.** Upstream forbids JetBrains Mono as a blanket "dev" font. Label Suite's `--font-mono` *is* JetBrains Mono (`tokens.css` line 58), and diagrams must match the product UI, so the ban is disabled here. It still applies to the `default` profile.
...
- **Serif + sans + mono**: three families, not more. If brand typography is all sans, keep Crimson Pro for `title` and `callout` anyway — the contrast is load-bearing.
