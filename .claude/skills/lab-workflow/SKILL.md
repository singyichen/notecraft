...
6. **寫進筆記**：用 `templates/lab-section.md` 的段落順序，每個實驗掛 `@ai-reference` 到講義步驟頁；LTspice 結果另立「進階」小節放理論 vs 模擬對照表；附錄收 CircuitJS 文字檔。寫完跑粗體自檢 grep、「中文為主」自檢 grep（兩條指令見 CLAUDE.md 工作慣例）、remark 清單檢查、`astro build`、preview 截圖。
