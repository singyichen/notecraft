...
- `**粗體（英文術語）**` 這種右括號／`)` 收尾的粗體，只要後面沒有空格或標點、直接接中文字，remark 就不會把收尾的 `**` 當合法收尾（CommonMark 的 right-flanking 規則：`**` 前面是標點時，後面必須接空白或標點才能收尾），整段會照字面輸出 `**文字**`；多組這樣的粗體連續出現時，失效的收尾標記還會被下一組頂替，造成整段 bold 範圍位移一格。單一術語只要在 `）**`／`)**` 後面補一個半形空格即可修正（不影響排版）。筆記正文（非 `@ai-visualize` 生成元件）若要並列 3 個以上「名稱＋定義＋範例」的平行項目，改用 Markdown 表格，不要在同一段塞多組 `**粗體**`
...
- 生成或改寫「機器學習實作系列」任一篇筆記的正文內容時，一律搭配 `.claude/skills/hung-yi-lee/SKILL.md`（李宏毅式教學風格：直覺先行、black box 到機制、生活化類比、預判學生疑問），該 skill 只影響教學語氣與講解結構，不影響 `@ai-visualize`／`@ai-reference` 標記與元件生成流程的技術規則
