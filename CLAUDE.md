...
- 粗體的**開頭**標記也會因同一條 right/left-flanking 規則失效：`中文**$x$ 說明**` 這種「緊接中文、後面接 `$`」的寫法，`**` 前是中文（非空白非標點）、後是標點 `$`，不符合 left-flanking，整段會照字面輸出 `**文字**`（數學本身仍會正常渲染，所以只看公式沒問題不代表沒事）。在 `**` 前補一個半形空格即可。自檢：`grep -rnoP '[\p{Han}A-Za-z0-9]\*\*\$' <筆記路徑>` 應為 0；也可在 `astro build` 後用 `grep -ao '\*\*' dist/notes/<slug>/index.html | wc -l` 反查字面殘留（程式碼區塊裡的 `**kwargs` 屬正常，需人工排除）
...
- 整理「電子學實作系列」的實驗講義（`App_LabN.pdf`）、要在 CircuitJS／Tinkercad／LTspice 模擬課程電路、或作者提到預報／結報／實驗數據記錄時，一律走 `lab-workflow` Skill（`.claude/skills/lab-workflow/SKILL.md`）：講義逐頁詳解放講義週筆記、實驗手冊放實驗週筆記，每個實驗固定「CircuitJS 看懂 → Tinkercad 預演 → LTspice 預報 → 實測結報」，網表與電路文字檔放 `simulations/<lab>/`，共用工具在 `simulations/tools/`
